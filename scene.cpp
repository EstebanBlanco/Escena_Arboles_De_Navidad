...
namespace Imager
{
    // Vacía la lista de objetos sólida y destruye o libera
    // los objetos sólidos que se encontraban en ella.
    ...
    // Un límite de cuán profundamente en recursión Calcular la iluminación puede ir
    //antes de que se rinda, a fin de evitar el desbordamiento de la pila de llamadas.
    const int MAX_OPTICAL_RECURSION_DEPTH = 20;

    /*Un límite de cuán débil puede ser la intensidad de un rayo de luz roja, 
    verde o azul después de llamadas recursivas desde múltiples reflexiones y/o 
    refracciones antes de darse por vencido. Esta intensidad se considera 
    demasiado débil para hacer una diferencia significativa en la imagen.*/
